...
class ALBEF(nn.Module):
    ...
    def forward(self, image, text, alpha=0, idx=None, beta=1.):
        ...
        rep_pos_ids = text.rep_ids
        batch_size, num_neg_text, text_len = rep_token_ids.shape
        extend_rep_pos_ids = rep_pos_ids.unsqueeze(1).repeat(1, num_neg_text, 1).view(-1, text_len)
        
        word_feat = F.normalize(self.text_proj(text_embeds[rep_pos_ids]), dim=-1)  # B,C
        ...
        text_neg_output = self.text_encoder(rep_token_ids, attention_mask=rep_text_attn_mask,
                                                return_dict=True, mode='text')
        word_neg_feat = F.normalize(self.text_proj(text_neg_output.last_hidden_state[extend_rep_pos_ids]), dim=-1)
        word_neg_feat = word_neg_feat.view(batch_size, num_neg_text, -1)  # B,num_neg,C
        word_neg_feat_all = torch.cat([word_feat.unsqueeze(1), word_neg_feat], dim=1)  # B,1+num_neg,C
        ...
        with torch.no_grad():
            ...
            word_feat_m = F.normalize(self.text_proj_m(text_output_m.last_hidden_state[rep_pos_ids]), dim=-1)  # B,C
            text_neg_output_m = self.text_encoder_m(rep_token_ids, attention_mask=rep_text_attn_mask,
                                                    return_dict=True, mode='text')
            word_neg_feat_m = F.normalize(self.text_proj_m(text_neg_output_m.last_hidden_state[extend_rep_pos_ids]), dim=-1)
            word_neg_feat_m = word_neg_feat_m.view(batch_size, num_neg_text, -1)  # B,num_neg,C
            word_neg_feat_m_all = torch.cat([word_feat_m.unsqueeze(1), word_neg_feat_m], dim=1)  # B,1+num_neg,C
            sim_neg_targets = torch.zeros((batch_size, 1 + num_neg_text), device=image.device)
            sim_neg_targets[:, 0] = 1
            
            sim_neg_mm = torch.matmul(image_feat_m.unsqueeze(1), word_neg_feat_m_all.transpose(1,2)).squeeze(1)
            sim_neg_targets = alpha * F.softmax(sim_neg_mm/self.temp1, dim=1) + (1 - alpha) * sim_neg_targets
        ...
        ##================= MLM ========================##
        # image_embeds: B,N,C
        # word_neg_feat_all: B,1+num_neg,C
        sim_neg = torch.matmul(image_feat.unsqueeze(1), word_neg_feat_m_all.transpose(1,2)).squeeze(1)
        sim_neg_m = torch.matmul(image_feat_m.unsqueeze(1), word_neg_feat_all.transpose(1,2)).squeeze(1)
#         loss_mlm = -torch.sum(F.log_softmax(sim_neg/self.temp1, dim=1) * sim_neg_targets, dim=1)[text.loss_mask].sum() / text.loss_mask.sum()
#         loss_mlm_m = -torch.sum(F.log_softmax(sim_neg_m/self.temp1, dim=1) * sim_neg_targets, dim=1)[text.loss_mask].sum() / text.loss_mask.sum()
#         loss_mlm = (loss_mlm + loss_mlm_m) / 2.
        
        margin = 0.
        loss_neg = (sim_neg[:, 1:].mean(-1, keepdim=True) - sim_neg[:, 0:1] + margin).clamp_(0.).sum(-1).mean()
        loss_neg_m = (sim_neg_m[:, 1:].mean(-1, keepdim=True) - sim_neg_m[:, 0:1] + margin).clamp_(0.).sum(-1).mean()
        loss_mlm = (loss_neg + loss_neg_m)
        
        return {'loss_ita': loss_ita, 'loss_tec': loss_mlm * beta}
