...
import pdb
from loss.loss_objects import matching_contrastive_loss_row, matching_contrastive_loss_col
import torch
...
class ALBEF(nn.Module):
    ...
    def forward(self, image, text, alpha=0):
        ...
        image_hidden_states = self.visual_encoder(image, output_hidden_states=True)
        image_feat = F.normalize(self.vision_proj(image_hidden_states[-1][:, 0, :]), dim=-1)  # B,C
        ...
        concept_embeds = self.concepter(image_hidden_states[:-1])
        concept_feat = F.normalize(self.concept_proj(concept_embeds), dim=-1)  # B,num_concept,C

        # get momentum features
        with torch.no_grad():
            self._momentum_update()
            image_hidden_states_m = self.visual_encoder_m(image, output_hidden_states=True)
            image_feat_m = F.normalize(self.vision_proj_m(image_hidden_states_m[-1][:, 0, :]), dim=-1)
            image_feat_all = torch.cat([image_feat_m.t(), self.image_queue.clone().detach()], dim=1)

            text_output_m = self.text_encoder_m(text.input_ids, attention_mask=text.attention_mask, return_dict=True,
                                                mode='text')
            text_projs_m = F.normalize(self.text_proj_m(text_output_m.last_hidden_state), dim=-1)
            text_feat_m = text_projs_m[:, 0].contiguous()
            word_feat_m = text_projs_m[:, 1:].contiguous()
            ...
            # 对concept和word进行配对
            concept_embeds_m = self.concepter_m(image_hidden_states_m[:-1])
            concept_feat_m = F.normalize(self.concept_proj_m(concept_embeds_m), dim=-1)
        ...
        # visual concept loss
        # gather features
        concept_feat_gather = AllGather.apply(concept_feat)
        word_feat_gather = AllGather.apply(word_feat)
        text_mask = (text.attention_mask[:, 1:] * text.content_mask[:, 1:]).contiguous()
        text_mask_gather = AllGather.apply(text_mask)
        concept_feat_m_gather = concat_all_gather(concept_feat_m)
        word_feat_m_gather = concat_all_gather(word_feat_m)
        rank_id = torch.distributed.get_rank()

        loss_vca_row = matching_contrastive_loss_row(concept_feat_gather, word_feat, text_mask, rank_id,
                                             self.temp_cpt, concept_feat_m_gather, word_feat_m, alpha)
        loss_vca_col = matching_contrastive_loss_col(concept_feat, word_feat_gather, text_mask_gather, rank_id,
                                             self.temp_cpt, concept_feat_m, word_feat_m_gather, alpha)
        loss_vca = (loss_vca_row + loss_vca_col) / 2.
