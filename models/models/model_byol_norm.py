...
class ALBEF(nn.Module):
    ...
    def forward(self, image, text, phrase, alpha=0, idx=None):
        ...
        # image feat
        image_embeds = self.visual_encoder.forward_post(torch.cat([image_mid_embeds, obj_embeds.detach()], dim=1),
                                                        start_layer=6)
        ...
        with torch.no_grad():
            phrase_embeds = self.text_encoder(phrase_ids, attention_mask=phrase_att_mask, return_dict=True,
                                              mode='text', output_hidden_states=True).hidden_states[-7][:, 0].view(
                num_batch, num_phrase, -1)
            phrase_embeds = F.normalize(phrase_embeds, dim=-1)
            sim_op_targets = align_obj_phrase(obj_embeds, phrase_embeds, phrase_mask)
        ...
        # 也可以尝试用momentum encoder得到obj,phrase embeds，然后对称计算loss
        loss_opa = (1 - self.cos(obj_aln_embeds, phrase_aln_embeds.detach()).mean()) * 5
        ...
        with torch.no_grad():
            ...
            image_embeds_m = self.visual_encoder_m.forward_post(torch.cat([image_mid_embeds_m, obj_embeds_m], dim=1),
                                                                start_layer=6)
        ...
        return {'loss_ita': loss_ita, 'loss_opa': loss_opa}
    ...
    def encode_image_phrase(self, image, phrase_embeds, phrase_masks):
        image_mid_embeds = self.visual_encoder.forward_pre(image, stop_layer=6)
        image_atts = torch.ones(image_mid_embeds.size()[:-1], dtype=torch.long).to(image.device)
        image_phrase_atts = torch.cat([image_atts, phrase_masks], dim=1)
        image_embeds = self.visual_encoder.forward_post(torch.cat([image_mid_embeds, phrase_embeds], dim=1),
                                                        start_layer=6, mask=image_phrase_atts)
        image_feat = F.normalize(self.vision_proj(image_embeds[:, 0, :]), dim=-1)
        return image_feat
