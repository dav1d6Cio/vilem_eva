...
from models.bert_sigmoid import BertConfig, BertTECModel
...
#         self.text_encoder.tec.load_state_dict(torch.load('weights/tec_param.pt'))
    ...
    def forward(self, image, text, alpha=0, idx=None):
        ...
        mlm_output, det_scores = self.text_encoder(text.rep_token_ids,
                                                 attention_mask=text.attention_mask,
...
#                                                  detection_labels=det_labels,
                                                 alpha=alpha,
                                                 return_det_scores=True
                                                 )

#         det_scores = torch.softmax(det_scores, dim=-1)[:, :, 1]  # 64,30
        loss_det = self.compute_ted_loss(det_scores[...,0], det_labels, pos_type='mean', neg_type='mean')
        loss_mlm = mlm_output.loss

        return {'loss_i2t': loss_i2t/2., 'loss_t2i': loss_t2i/2., 'loss_tec': loss_mlm, 'loss_det': loss_det * self.lambda_ted}

    def compute_ted_loss(self, det_scores, det_labels, pos_type='mean', neg_type='mean'):
        valid_index = ((det_labels==0).sum(-1)>0) & ((det_labels==1).sum(-1)>0)  # 64
        det_scores = det_scores[valid_index]
        det_labels = det_labels[valid_index]
        pos_scores = torch.zeros_like(det_scores)
        neg_scores = torch.zeros_like(det_scores)
        pos_scores[det_labels==1] = det_scores[det_labels==1]
        neg_scores[det_labels==0] = det_scores[det_labels==0]

        if pos_type=='mean':
            pos_score = pos_scores.sum(-1) / (det_labels==1).sum(-1)
        elif pos_type=='min':
            pos_score = pos_scores.min(-1)

        if neg_type=='mean':
            neg_score = neg_scores.sum(-1) / (det_labels==0).sum(-1)
        elif pos_type=='max':
            neg_score = neg_scores.max(-1)

        loss_det = (self.margin - pos_score + neg_score).clamp(min=0)
        return loss_det.sum()
