...
#         self.register_buffer("text_mask_queue", torch.zeros((30, self.queue_size), dtype=torch.long))
    ...
    def forward(self, image, text, alpha=0, idx=None, beta=1.):
        ...
        image_unorm_feat = self.vision_proj(image_embeds[:, 0, :]) # N,C
        image_feat = F.normalize(image_unorm_feat, dim=-1)
        mix_index = torch.tensor(list(range(1,64))+[0]).long()
        mix_image_unorm_feat = image_unorm_feat + image_unorm_feat[mix_index]
        mix_image_feat = F.normalize(mix_image_unorm_feat, dim=-1)
        image_feat = torch.cat([image_feat, mix_image_feat], dim=0)
        ...
        mix_text_unorm_feat = text_unorm_feat + text_unorm_feat[mix_index]
        mix_text_feat = F.normalize(mix_text_unorm_feat, dim=-1)
        text_feat = torch.cat([text_feat, mix_text_feat], dim=0)
        ...
        with torch.no_grad():
            ...
            mix_image_unorm_feat_m = image_unorm_feat_m + image_unorm_feat_m[mix_index]
            mix_image_feat_m = F.normalize(mix_image_unorm_feat_m, dim=-1)
            image_feat_m = torch.cat([image_feat_m, mix_image_feat_m], dim=0)
            ...
            mix_text_unorm_feat_m = text_unorm_feat_m + text_unorm_feat_m[mix_index]
            mix_text_feat_m = F.normalize(mix_text_unorm_feat_m, dim=-1)
            text_feat_m = torch.cat([text_feat_m, mix_text_feat_m], dim=0)
