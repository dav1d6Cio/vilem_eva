...
class ALBEF(nn.Module):
    ...
    def forward(self, image, text, alpha=0, idx=None, beta=1.):
        ...
        with torch.no_grad():
            ...
            # forward bad captions
            bad_cap_embeds = self.text_encoder_m(text.rep_token_ids, attention_mask=text.attention_mask,
                                                return_dict=True, mode='text').last_hidden_state
            bad_cap_feat_m = F.normalize(self.text_proj_m(bad_cap_embeds[:, 0, :]), dim=-1)
        ...
        sim_i2t_clone = sim_i2t.clone() * self.temp
        sim_i2t_clone.fill_diagonal_(-1)
        max_sim = sim_i2t_clone.max(-1)[0]
        
        sim_i2t_current = (image_feat @ text_feat_m.t()).diagonal()
        sim_i2t_m_current = (image_feat @ bad_cap_feat_m.t()).diagonal()
        
        loss_mask = sim_i2t_m_current > max_sim
        loss_rank = (sim_i2t_m_current - sim_i2t_current).clamp_(0.)[loss_mask].sum()
        

        return {'loss_ita': loss_ita, 'loss_rank': loss_rank * beta}
