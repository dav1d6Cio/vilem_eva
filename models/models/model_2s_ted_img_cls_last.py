...
from models.bert_blip_wo_ca import BertConfig, BertTECModel
...
class ALBEF(nn.Module):
    ...
    def forward(self, image, text, alpha=0, idx=None, beta=0.):
        ...
        with torch.no_grad():
            ...
            image_embeds_list_m = self.visual_encoder_m(image, return_all_hidden_states=True)
            image_embeds_m = image_embeds_list_m[-1]
...
#         mlm_output, _ = self.text_encoder(text.rep_token_ids,
#                                        attention_mask=text.attention_mask,
...
#                                        )
        ...
        mlm_output, loss_det = self.text_encoder(text.rep_token_ids,
                                        attention_mask=text.attention_mask,
                                        encoder_hidden_states=image_embeds_list_m[-2],
                                        encoder_attention_mask=image_atts,
                                        return_dict=True,
        ...
        mlm_output_m, loss_det_m = self.text_encoder_m(text.rep_token_ids,
                                        attention_mask=text.attention_mask,
        ...
        loss_mlm_m = mlm_output_m.loss
        
        loss_mlm = (loss_mlm + loss_mlm_m) / 2.
        loss_det = (loss_det + loss_det_m) / 2.

        return {'loss_ita': loss_ita, 'loss_tec': loss_mlm, 'loss_det': loss_det * self.lambda_ted}
