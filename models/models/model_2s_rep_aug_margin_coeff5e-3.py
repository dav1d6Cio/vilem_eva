...
#         rep_text_attn_mask = text.attention_mask.unsqueeze(1).repeat(1, num_neg_text, 1).view(-1, text_len)
        ...
        loss_neg = (loss_neg + loss_neg_m) * 5e-3
#         ita_grad = torch.autograd.grad(outputs=loss_ita, inputs=self.text_proj.weight, retain_graph=True)
#         neg_grad = torch.autograd.grad(outputs=loss_neg, inputs=self.text_proj.weight, retain_graph=True)
#         pdb.set_trace()
