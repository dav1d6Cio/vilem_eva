...
from models.vit_group import VisionTransformer
...
class ALBEF(nn.Module):
    ...
    def forward(self, image, text, alpha, idx):
        ...
        obj_feat = F.normalize(self.vision_proj(image_embeds[:, -self.num_obj_query:, :].mean(1)), dim=-1)  # B,num_obj,C
        ...
        with torch.no_grad():
            ...
            obj_feat_m = F.normalize(self.vision_proj_m(image_embeds_m[:, -self.num_obj_query:, :].mean(1)), dim=-1)
            obj_feat_all = torch.cat([obj_feat_m.t(), self.obj_queue.clone().detach()], dim=1)
            ...
            if self.distill:
                ...
                sim_o2t_m = obj_feat_m @ text_feat_all / self.temp
                sim_t2o_m = text_feat_m @ obj_feat_all / self.temp
        ...
        sim_o2t = obj_feat @ text_feat_all / self.temp
        sim_t2o = text_feat @ obj_feat_all / self.temp
        ...
        self._dequeue_and_enqueue(image_feat_m, text_feat_m, obj_feat_m, idx)

        return loss_ita, loss_ota
