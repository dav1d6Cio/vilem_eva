...
class ALBEF(nn.Module):
    def __init__(self,
        ...
        self.add_ted_head = config['add_ted_head'] if 'add_ted_head' in config.keys() else False
        self.add_mlm_head = config['add_mlm_head'] if 'add_mlm_head' in config.keys() else False
        ...
        self.text_encoder = BertTECModel.from_pretrained(text_encoder, config=bert_config,
                                                         add_ted_head=self.add_ted_head,
                                                         add_mlm_head=self.add_mlm_head,
                                                         add_cls_ted_head=self.add_ted_head,
                                                         add_cls_mlm_head=self.add_mlm_head)
        
        self.copy_head_params()
    ...
    def forward(self, image, text, alpha=0, idx=None, beta=1.):
        ...
        sim_i2t_pre = image_feat @ text_feat_all
        sim_t2i_pre = text_feat @ image_feat_all
        sim_i2t = sim_i2t_pre / self.temp
        sim_t2i = sim_t2i_pre / self.temp
...
#         pdb.set_trace()
        ...
        loss_mlm_l = (loss_mlm1 + loss_mlm2) / 2.
        loss_mlm_g = (loss_mlm3 + loss_mlm4) / 2. * beta
        loss_det_l = (loss_det1 + loss_det2) / 2.
    ...
    @torch.no_grad()
    def copy_head_params(self):
        # 需要将self.text_encoder.cls.predictions.transform的权重copy到self.text_encoder.cls_mlm.predictions.transform,
        # self.text_encoder.tec.transform, self.text_encoder.cls_ted.transform中
        self.copy_params_pair(self.text_encoder.cls.predictions.transform, self.text_encoder.cls_mlm.predictions.transform)
        self.copy_params_pair(self.text_encoder.cls.predictions.transform, self.text_encoder.tec.transform)
        self.copy_params_pair(self.text_encoder.cls.predictions.transform, self.text_encoder.cls_ted.transform)
        
    @torch.no_grad()
    def copy_params_pair(self, module_src, module_tgt):
        # src: 预训练好的module; tgt: 未预训练的module
        # param: 预训练好的权重；param_m: 未经预训练的权重
        for param, param_m in zip(module_src.parameters(), module_tgt.parameters()):
            param_m.data.copy_(param.data)  # initialize
