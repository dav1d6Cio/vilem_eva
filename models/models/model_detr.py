...
class ALBEF(nn.Module):
    def __init__(self,
        ...
        self.global_fn = nn.Linear(vision_width, embed_dim)
        ...
        # object
        self.global_fn_m = nn.Linear(vision_width, embed_dim)
        ...
        self.model_pairs = [[self.visual_encoder, self.visual_encoder_m],
                            ...
                            [self.global_fn, self.global_fn_m],
    ...
    def forward(self, image, text, alpha=0, idx=None):
        ...
        image_feat = F.normalize(self.extract_local_global_feat(image_embeds), dim=-1)

        text_output = self.text_encoder(text.input_ids, attention_mask=text.attention_mask, return_dict=True, mode='text')
        text_embeds = self.compute_text_mean_feat(text_output.last_hidden_state, text.attention_mask)
        ...
        with torch.no_grad():
            ...
            image_feat_m = F.normalize(self.extract_local_global_feat_m(image_embeds_m), dim=-1)
            ...
            text_embeds_m = self.compute_text_mean_feat(text_output_m.last_hidden_state, text.attention_mask)
            text_feat_m = F.normalize(self.text_proj_m(text_embeds_m), dim=-1)
    ...
    def extract_local_global_feat(self, image_embeds):
    ...
    def extract_local_global_feat_m(self, image_embeds):
        patch_embeds = image_embeds[:, 1:]
        B, N, C = patch_embeds.shape
        ...
        f_global = self.global_fn_m(f_slots.mean(dim=1))
    ...
    def compute_text_mean_feat(self, text_embed, text_mask, eps=1e-8):
        # text_embed: B,L,C
        # text_mask: B,L
        new_text_mask = self.get_new_mask(text_mask).unsqueeze(-1)  # B,L,1
        text_embed = text_embed * new_text_mask # B,L,C
        text_feat = text_embed.sum(1) / (new_text_mask.sum(1) + eps)  # B,C
        return text_feat
