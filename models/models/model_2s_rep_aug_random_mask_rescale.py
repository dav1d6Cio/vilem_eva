...
class ALBEF(nn.Module):
    ...
    def forward(self, image, text, alpha=0, idx=None):
        ...
        with torch.no_grad():
            ...
            rep_token_ids = text.input_ids.clone().unsqueeze(1).repeat(1, num_neg_text, 1)
            rep_token_ids = rep_token_ids.view(-1, text_len)
            probability_matrix = torch.full(rep_token_ids.shape, 0.15)
            rep_token_ids = self.mask(rep_token_ids, self.text_encoder.config.vocab_size, image.device,
            ...
            #             sim_neg_m = torch.matmul(image_feat_m.unsqueeze(1), text_neg_feat_all.transpose(1,2)).squeeze(1) / 0.1 # B,1+num_neg
            ...
            sim_neg_targets[:, 0] = 1
        #             sim_neg_targets = alpha * F.softmax(sim_neg_m, dim=1) + (1 - alpha) * sim_neg_targets
        ...
        # sim_neg = torch.matmul(image_feat.unsqueeze(1), text_neg_feat_all.transpose(1, 2)).squeeze(1) / self.temp_neg
        # loss_neg = -torch.sum(F.log_softmax(sim_neg, dim=1) * sim_neg_targets, dim=1)[text.loss_mask].sum() / text.loss_mask.sum()

        # margin loss for rep words
        margin = 0.
        ...
        loss_neg = (sim_neg[:, 1:] - sim_neg[:, 0:1] + margin).clamp_(0.).sum(-1).mean()
        loss_neg_m = (sim_neg_m[:, 1:] - sim_neg_m[:, 0:1] + margin).clamp_(0.).sum(-1).mean()
        loss_neg = (loss_neg + loss_neg_m)
        loss_neg_detach = loss_neg.detach()
        loss_neg_rescale = loss_neg_detach * loss_neg
        
#         ita_grad = torch.autograd.grad(outputs=loss_ita, inputs=sim_neg, retain_graph=True)
#         neg_grad = torch.autograd.grad(outputs=loss_neg_rescale, inputs=sim_neg, retain_graph=True)
#         pdb.set_trace()
        self._dequeue_and_enqueue(image_feat_m, text_feat_m, idx)

        return {'loss_ita': loss_ita, 'loss_neg_d': loss_neg_detach, 'loss_neg_r':loss_neg_rescale}  #
