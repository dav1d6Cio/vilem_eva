...
class ALBEF(nn.Module):
    ...
    def forward(self, image, text, alpha=0, idx=None):
        ...
        correction_label = self.tec_mask(text.rep_ids, labels)

        if 'sw_mask' in text.keys():
            correction_label[text.sw_mask == 0] = -100
        # with torch.no_grad():
        ...
        mlm_output = self.text_encoder(text.rep_token_ids,
                                                 attention_mask=text.attention_mask,
        ...
        return {'loss_i2t': loss_i2t / 2., 'loss_t2i': loss_t2i / 2., 'loss_tec': loss_mlm}
    ...
    def tec_mask(self, rep_ids, targets):
        targets[targets==self.tokenizer.cls_token_id] = -100
        targets[targets==self.tokenizer.pad_token_id] = -100
        targets[(rep_ids==0) & (targets!=-100)] = self.tokenizer.correct_token_id
