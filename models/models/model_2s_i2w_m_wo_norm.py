...
class ALBEF(nn.Module):
    ...
    def forward(self, image, text, alpha=0, idx=None, beta=1.):
        ...
        rep_word_feat = rep_text_embeds[:, 1:, :]  # N,L-1,C
        ...
        with torch.no_grad():
            ...
            rep_text_embeds_m = self.text_encoder_m(text.rep_token_ids, attention_mask=text.attention_mask,
                                                     return_dict=True, mode='text').last_hidden_state
            rep_word_feat_m = rep_text_embeds_m[:, 1:, :]  # N,L-1,C
        ...
        sim_i2w = torch.matmul(rep_word_feat_m, image_embeds[:, 0, :].unsqueeze(-1)).squeeze(-1)  # N,L-1
        sim_i2w_m = torch.matmul(rep_word_feat, image_embeds_m[:, 0, :].unsqueeze(-1)).squeeze(-1)
        ...
        if 0 in sim_i2w_targets.sum(-1):
            ...
            sim_i2w_m = sim_i2w_m[non_zero_idx]
            sim_i2w_targets = sim_i2w_targets[non_zero_idx]
        loss_i2w = -torch.log((F.softmax(sim_i2w, dim=-1) * sim_i2w_targets).sum(1)).mean()
        loss_i2w_m = -torch.log((F.softmax(sim_i2w_m, dim=-1) * sim_i2w_targets).sum(1)).mean()
