...
from models.bert_bridge import BridgeModel
...
class ALBEF(nn.Module):
    def __init__(self,
        ...
        bridge_config = copy.deepcopy(bert_config)
        bridge_config.num_hidden_layers = 6
        self.bridger = BridgeModel.from_pretrained(text_encoder, config=bridge_config, embeddings=self.text_encoder.embeddings)
        ...
        self.bridger_m = BridgeModel.from_pretrained(text_encoder, config=bridge_config,
                                                   embeddings=self.text_encoder_m.embeddings)
    ...
    def forward(self, image, text, alpha=0, idx=None, beta=1.):
        ...
        loss_mask = text.attention_mask.clone().float()
        loss_mask[:, 0] = 0

        if 'sw_mask' in text.keys():
            det_labels[text.sw_mask == 0] = -100
        
        rep_text_embeds = tuple([rep_text_embeds[i] for i in [1,3,5,7,9,11]])
        rep_text_embeds_m = tuple([rep_text_embeds_m[i] for i in [1,3,5,7,9,11]])
        image_embeds_list = [image_embeds_list[i] for i in [2,4,6,8,10,12]]
        image_embeds_m_list = [image_embeds_m_list[i] for i in [2,4,6,8,10,12]]
        # momentum visual feat + text encoder
        v_rep_text_embeds = self.bridger(rep_text_embeds,
                                       attention_mask=text.attention_mask,
                                       encoder_hidden_states=image_embeds_m_list,
                                       encoder_attention_mask=image_atts,
                                       return_dict=True
                                       ).last_hidden_state

        # visual feat + momentum text encoder
        v_rep_text_embeds_m = self.bridger(rep_text_embeds_m,
                                       attention_mask=text.attention_mask,
                                       encoder_hidden_states=image_embeds_list,
                                       encoder_attention_mask=image_atts,
                                       return_dict=True
                                       ).last_hidden_state

        bce_logits = torch.matmul(rep_text_embeds[-1].unsqueeze(-2), v_rep_text_embeds.unsqueeze(-1)).squeeze()  # N,L
        bce_logits_m = torch.matmul(rep_text_embeds_m[-1].unsqueeze(-2), v_rep_text_embeds_m.unsqueeze(-1)).squeeze()  # N,L
        loss_det = nn.BCEWithLogitsLoss(reduction='none')(bce_logits, det_labels.float())
        loss_det = (loss_det * loss_mask).sum() / loss_mask.sum()
        loss_det_m = nn.BCEWithLogitsLoss(reduction='none')(bce_logits_m, det_labels.float())
        loss_det_m = (loss_det_m * loss_mask).sum() / loss_mask.sum()

        loss_det = (loss_det + loss_det_m) / 2.

        return {'loss_ita': loss_ita, 'loss_det': loss_det}
...
class MultiHeadCrossAttention(nn.Module):
    ...
    def invert_attention_mask(self, encoder_attention_mask):
        ...
        encoder_extended_attention_mask = encoder_extended_attention_mask.float()  # fp16 compatibility
