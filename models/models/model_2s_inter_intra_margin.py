...
#         self.temp1 = nn.Parameter(torch.ones([]) * config['temp'])
        ...
        self.text_encoder_m = BertTECModel.from_pretrained(text_encoder, config=bert_config, add_ted_head=False, add_mlm_head=False)
    ...
    def forward(self, image, text, alpha=0, idx=None, beta=1.):
        ...
        loss_neg_m = (sim_neg_m[:, 1:] - sim_neg_m[:, 0:1] + margin).clamp_(0.).sum(-1)[non_zero_idx].mean()
        loss_neg = (loss_neg + loss_neg_m) * 0.08
        ...
        mean_r_score = (sim_i2w * right_word_mask).sum(-1) / (right_word_mask.sum(-1) + 1e-6)
        mean_r_score_m = (sim_i2w_m * right_word_mask).sum(-1) / (right_word_mask.sum(-1) + 1e-6)
        mean_w_score = (sim_i2w * wrong_word_mask).sum(-1) / (wrong_word_mask.sum(-1) + 1e-6)
        mean_w_score_m = (sim_i2w_m * wrong_word_mask).sum(-1) / (wrong_word_mask.sum(-1) + 1e-6)
        
        non_zero_idx = text.rep_ids.sum(-1)>0
        if False in non_zero_idx:
            mean_r_score = mean_r_score[non_zero_idx]
            mean_r_score_m = mean_r_score_m[non_zero_idx]
            mean_w_score = mean_w_score[non_zero_idx]
            mean_w_score_m = mean_w_score_m[non_zero_idx]
        
        if 0 in right_word_mask.sum(-1):
            non_zero_idx = right_word_mask.sum(-1) > 0
            mean_r_score = mean_r_score[non_zero_idx]
            mean_r_score_m = mean_r_score_m[non_zero_idx]
            mean_w_score = mean_w_score[non_zero_idx]
            mean_w_score_m = mean_w_score_m[non_zero_idx]
        ...
        margin = 0.01
        loss_i2w = (mean_w_score - mean_r_score + margin).clamp_(0.).mean()
        loss_i2w_m = (mean_w_score_m - mean_r_score_m + margin).clamp_(0.).mean()
        loss_i2w = (loss_i2w + loss_i2w_m) * 0.1
