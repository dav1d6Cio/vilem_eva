...
#         pdb.set_trace()
        ...
        neg_det_labels, pos_det_labels, correction_labels = self.tec_mask(text.rep_token_ids, text.rep_ids, labels)
        # with torch.no_grad():
        ...
        mlm_output, neg_det_scores = self.text_encoder(text.rep_token_ids,
                                       attention_mask=text.attention_mask,
        ...
        _, pos_det_scores = self.text_encoder(text.input_ids,
                                            attention_mask=text.attention_mask,
                                            encoder_hidden_states=image_embeds_list[2:-1],
                                            encoder_attention_mask=image_atts,
                                            return_dict=True,
        ...
        loss_det = (self.margin - pos_det_scores[:, :, 1] + neg_det_scores[:, :, 1]).clamp(min=0)  # B,L
        valid_word_mask = text.rep_ids==1
        if 'sw_mask' in text.keys():
            valid_word_mask[text.sw_mask == 0] = False
        loss_det = loss_det[valid_word_mask].sum() / valid_word_mask.sum()


        loss_mlm = mlm_output.loss

        return {'loss_ita': loss_ita, 'loss_tec': loss_mlm, 'loss_det': loss_det}
    ...
    def tec_mask(self, input_ids, rep_ids, targets):
        neg_det_labels = torch.full_like(input_ids, -100)  # N,L
        neg_det_labels[rep_ids == 1] = 0
        pos_det_labels = torch.full_like(input_ids, -100)  # N,L
        pos_det_labels[rep_ids == 1] = 1

        correction_labels = torch.full_like(targets, -100)
        correction_labels[rep_ids==1] = targets[rep_ids==1]

        return neg_det_labels, pos_det_labels, correction_labels
