...
class ALBEF(nn.Module):
    ...
    def forward(self, image, text, alpha=0):
        ...
        gen_text_embeds = image_embeds[:, -24:].contiguous()
        # LM
        prediction_scores = self.text_encoder.cls(gen_text_embeds)
        pdb.set_trace()
        gen_text_labels = text.input_ids.clone()
        gen_text_labels = gen_text_labels[:, 1:].contiguous()  # remove CLS
        loss_fct = nn.CrossEntropyLoss()  # -100 index = padding token
        loss_lm = loss_fct(prediction_scores.view(-1, self.tokenizer.vocab_size), gen_text_labels.view(-1))
