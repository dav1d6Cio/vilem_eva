...
class ALBEF(nn.Module):
    ...
    def forward(self, image, text, alpha=0, idx=None):
        ...
        if idx is not None:
            ...
            sim_targets_t2i = pos_idx / pos_idx.sum(1, keepdim=True)

            sim_targets_i2t = torch.cat([sim_targets_t2i[:, :batch_size],
                                         torch.zeros_like(sim_targets_t2i[:, :batch_size]),
                                         sim_targets_t2i[:, batch_size:],
                                         ], dim=-1)
        ...
        with torch.no_grad():
            ...
            # negative captions
            text_neg_output_m = self.text_encoder_m(text.rep_token_ids, attention_mask=text.attention_mask,
                                                     return_dict=True, mode='text')
            text_neg_feat_m = F.normalize(self.text_proj_m(text_neg_output_m.last_hidden_state[:, 0, :]), dim=-1)
            text_feat_all = torch.cat([text_feat_m.t(), text_neg_feat_m.t(),
            ...
            if idx is None:
                sim_targets_i2t = torch.zeros(sim_i2t_m.size()).to(image.device)
                sim_targets_i2t.fill_diagonal_(1)
                sim_targets_t2i = torch.zeros(sim_t2i_m.size()).to(image.device)
                sim_targets_t2i.fill_diagonal_(1)
            
            sim_mask = torch.zeros_like(sim_i2t_m)
            mask_idx = torch.where(text.rep_ids.sum(-1)<2)[0]
            sim_mask[mask_idx, mask_idx+batch_size] = -1e9

            sim_i2t_targets = alpha * F.softmax(sim_i2t_m+sim_mask, dim=1) + (1 - alpha) * sim_targets_i2t
            sim_t2i_targets = alpha * F.softmax(sim_t2i_m, dim=1) + (1 - alpha) * sim_targets_t2i

        sim_i2t = image_feat @ text_feat_all / self.temp
        sim_t2i = text_feat @ image_feat_all / self.temp

        loss_i2t = -torch.sum(F.log_softmax(sim_i2t+sim_mask, dim=1) * sim_i2t_targets, dim=1).mean()
        loss_t2i = -torch.sum(F.log_softmax(sim_t2i, dim=1) * sim_t2i_targets, dim=1).mean()

        loss_ita = (loss_i2t + loss_t2i) / 2

        self._dequeue_and_enqueue(image_feat_m, text_feat_m, text_neg_feat_m, idx)
    ...
    @torch.no_grad()
    def _dequeue_and_enqueue(self, image_feat, text_feat, text_neg_feat, idx):
        # gather keys before updating queue
        image_feats = concat_all_gather(image_feat)
        text_feats = concat_all_gather(text_feat)
        text_neg_feats = concat_all_gather(text_neg_feat)
        ...
        self.text_neg_queue[:, ptr:ptr + batch_size] = text_neg_feats.T
        if idx is not None:
