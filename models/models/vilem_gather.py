...
class ALBEF(nn.Module):
    ...
    def forward(self, image, text, alpha=0, idx=None, beta=1.):
        ...
        image_embeds = image_embeds_list[-1] # N,num_patch,C
        if is_video:
            num_patch = image_embeds.shape[1]
            image_atts = torch.ones((B, num_patch + (num_patch-1)*(N-1)), dtype=torch.long).to(image.device)
            image_feat = self.vision_proj(image_embeds[:, 0, :]).view(B,N,-1).mean(1)
        ...
        # gather all feature for ITC
        image_feat_gather = AllGather.apply(image_feat)
        text_feat_gather = AllGather.apply(text_feat)

        if idx is not None:
            idx_gather = concat_all_gather(idx)
            idx_gather = idx_gather.view(-1, 1)
            idx_all = torch.cat([idx_gather.t(), self.idx_queue.clone().detach()], dim=1)
            pos_idx = torch.eq(idx_gather, idx_all).float()
#             pdb.set_trace()
        ...
        with torch.no_grad():
            ...
            if is_video:
                image_feat_m = self.vision_proj_m(image_embeds_m[:, 0, :]).view(B,N,-1).mean(1)
                image_feat_m = F.normalize(image_feat_m, dim=-1)
            else:
                image_feat_m = F.normalize(self.vision_proj_m(image_embeds_m[:, 0, :]), dim=-1)
            image_feat_gather_m = concat_all_gather(image_feat_m)
            image_feat_all = torch.cat([image_feat_gather_m.t(), self.image_queue.clone().detach()], dim=1)
            text_output_m = self.text_encoder_m.bert(text.input_ids, attention_mask=text.attention_mask,
                                                     return_dict=True, mode='text')
            text_feat_m = F.normalize(self.text_proj_m(text_output_m.last_hidden_state[:, 0, :]), dim=-1)
            text_feat_gather_m = concat_all_gather(text_feat_m)
            text_feat_all = torch.cat([text_feat_gather_m.t(), self.text_queue.clone().detach()], dim=1)

            sim_i2t_m = image_feat_gather_m @ text_feat_all / self.temp
            sim_t2i_m = text_feat_gather_m @ image_feat_all / self.temp
        ...
        sim_i2t = image_feat_gather @ text_feat_all / self.temp
        sim_t2i = text_feat_gather @ image_feat_all / self.temp

        loss_i2t = -torch.sum(F.log_softmax(sim_i2t, dim=1) * sim_i2t_targets, dim=1).mean()
        loss_t2i = -torch.sum(F.log_softmax(sim_t2i, dim=1) * sim_t2i_targets, dim=1).mean()

        loss_ita = (loss_i2t + loss_t2i) / 2.
        self._dequeue_and_enqueue(image_feat_gather_m, text_feat_gather_m, idx, text.input_ids, text.attention_mask)
        
#         ita_grad = torch.autograd.grad(outputs=loss_ita, inputs=sim_i2t_pre, retain_graph=True)
#         pdb.set_trace()
        ##================= MLM ========================##
        #         pdb.set_trace()
        ...
        if is_video:
            image_embeds_m_list = self.cat_frame_embeds_list(image_embeds_m_list, B, N)
            image_embeds_list = self.cat_frame_embeds_list(image_embeds_list, B, N)
        
        # TECD with local visual features
        ...
        # TECD with global visual features
    ...
    #, 'loss_tec_l': loss_mlm_l, 'loss_det_l': loss_det_l, 'loss_tec_g': loss_mlm_g, 'loss_det_g': loss_det_g
    ...
    @torch.no_grad()
    def _dequeue_and_enqueue(self, image_feats, text_feats, idx, text_ids, text_mask):
