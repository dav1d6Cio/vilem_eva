    ...
    def forward(self, image, text, alpha=0, idx=None):
        ...
        with torch.no_grad():
            ...
            text_neg_output_m = self.text_encoder_m(rep_token_ids, attention_mask=rep_text_attn_mask,
                                                    return_dict=True, mode='text')
            word_neg_feat_m = F.normalize(self.text_proj_m(text_neg_output_m.last_hidden_state[extend_rep_pos_ids]),
                                          dim=-1)
            word_neg_feat_m = word_neg_feat_m.view(batch_size, num_neg_text, -1)  # B,num_neg,C
            word_neg_feat_all = torch.cat([word_feat_m.unsqueeze(1), word_neg_feat_m], dim=1)  # B,1+num_neg,C
        ...
        text_neg_feat_all_online = torch.cat([word_feat.unsqueeze(1), word_neg_feat_m], dim=1)  # B,1+num_neg,C
        sim_neg = torch.matmul(image_feat.unsqueeze(1), text_neg_feat_all_online.transpose(1, 2)).squeeze(1) / 0.1
        loss_neg = -torch.sum(F.log_softmax(sim_neg, dim=1) * sim_neg_targets, dim=1)[
            text.loss_mask].sum() / text.loss_mask.sum()

        self._dequeue_and_enqueue(image_feat_m, text_feat_m, idx)

        return {'loss_ita': loss_ita, 'loss_neg': loss_neg}
