...
class ALBEF(nn.Module):
    def __init__(self,
        ...
        self.cross_attention1 = MultiHeadCrossAttention(config=bert_config)
        self.cross_attention2 = MultiHeadCrossAttention(config=bert_config)
    ...
    def forward(self, image, text, alpha=0, idx=None, beta=1.):
        ...
        v_rep_text_embeds = self.cross_attention1(rep_text_embeds, image_embeds_m[:, 1:], image_atts[:, 1:])

        # visual feat + momentum text encoder
        v_rep_text_embeds_m = self.cross_attention2(rep_text_embeds_m, image_embeds[:, 1:], image_atts[:, 1:])
