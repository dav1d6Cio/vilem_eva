...
class ALBEF(nn.Module):
    def __init__(self,
        ...
        self.word_vision_proj = nn.Linear(vision_width, embed_dim)
        ...
        self.word_proj_m = nn.Linear(text_width, embed_dim)
        self.word_vision_proj_m = nn.Linear(vision_width, embed_dim)

        self.model_pairs = [[self.visual_encoder, self.visual_encoder_m],
                            [self.vision_proj, self.vision_proj_m],
                            [self.text_encoder, self.text_encoder_m],
                            [self.text_proj, self.text_proj_m],
                            [self.word_proj, self.word_proj_m],
                            [self.word_vision_proj, self.word_vision_proj_m],
                            ]
    ...
    def forward(self, image, text, alpha=0, idx=None, beta=1.):
        ...
        word_image_feat = F.normalize(self.word_vision_proj(image_embeds[:, 0, :]), dim=-1)
        ...
        rep_word_feat = F.normalize(self.word_proj(rep_text_embeds[:, 1:, :]), dim=-1)  # N,L-1,C
        ...
        with torch.no_grad():
            ...
            word_image_feat_m = F.normalize(self.word_vision_proj_m(image_embeds_m[:, 0, :]), dim=-1)
            ...
            rep_text_embeds_m = self.text_encoder_m(text.rep_token_ids, attention_mask=text.attention_mask,
                                                     return_dict=True, mode='text').last_hidden_state
            rep_word_feat_m = F.normalize(self.word_proj_m(rep_text_embeds_m[:, 1:, :]), dim=-1)  # N,L-1,C
        ...
        sim_i2w = torch.matmul(rep_word_feat_m, word_image_feat.unsqueeze(-1)).squeeze(-1)  # N,L-1
        sim_i2w_m = torch.matmul(rep_word_feat, word_image_feat_m.unsqueeze(-1)).squeeze(-1)
