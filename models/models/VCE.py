...
from models.detr import VisualConceptExtractor
...
class ALBEF(nn.Module):
    def __init__(self,
        ...
        self.query_embed = nn.Embedding(config['num_queries'], vision_width)
        self.query_embed.weight.data.normal_(mean=0.0, std=0.02)
        self.visual_conceptor = VisualConceptExtractor(num_decoder_layers=6)

        bert_config = BertConfig.from_json_file(config['bert_config'])
        self.text_encoder, loading_info = BertModel.from_pretrained(text_encoder, config=bert_config, add_pooling_layer=False, output_loading_info=True)
        print(loading_info)
    ...
    def forward(self, image, text, alpha, idx):
        ...
        encoder_outputs = [image_layers[i] for i in range(1, 13, 2)]
        concept_feat = self.visual_conceptor(encoder_outputs, image_atts, self.query_embed.weight, image_layers[0])
    ...
    def forward_concept_contrastive(self, concept_feat, text_embeds, text_mask):
        ...
        word_feat = F.normalize(self.word_proj(text_embeds[:, 1:]), dim=-1)  # 去掉cls位
        word_mask = text_mask[:, 1:]
        ...
        word_mask_all = AllGather.apply(text_mask)[:, 1:]

        sim_i2t = self.get_sim_with_alignment(word_feat_all, concept_feat, word_mask_all, concept_mask)
        sim_t2i = self.get_sim_with_alignment(concept_feat_all, word_feat, concept_mask_all, word_mask)
    ...
    def max_region_sum_word(self, sequence_output, visual_output, attention_mask, video_mask):
        ...
        sim_matrix = alignments.max(-2)[0]  # B,B,L
        sim_matrix[sim_matrix < -1] = 0
        sim_matrix = sim_matrix.sum(-1)  # B,B
        return sim_matrix
