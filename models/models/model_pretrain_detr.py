...
class ALBEF(nn.Module):
    def __init__(self,
        ...
        self.concepter = Conceptformer(img_size=config['image_res'], patch_size=16, embed_dim=768, depth=12, num_heads=12,
            mlp_ratio=4, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), num_concept=16)
        ...
        self.visual_encoder_m = VisionTransformer(
            img_size=config['image_res'], patch_size=16, embed_dim=768, depth=12, num_heads=12,
            mlp_ratio=4, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6))
        self.concepter_m = Conceptformer(img_size=config['image_res'], patch_size=16, embed_dim=768, depth=12, num_heads=12,
                                    mlp_ratio=4, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), num_concept=16)
...
#         word_feat_m_gather = concat_all_gather(word_feat_m)
        loss_vca = matching_contrastive_loss(concept_feat, word_feat, text_mask,
                                             self.temp_cpt, concept_feat_m, word_feat_m, alpha)
