...
class ALBEF(nn.Module):
    ...
    def forward(self, image, text, phrase, alpha=0, idx=None):
        ...
        # image feat
        image_embeds = self.visual_encoder.forward_post(torch.cat([image_mid_embeds, obj_embeds.detach()], dim=1), start_layer=6)
        ...
        loss_opa = self.mse(obj_aln_embeds, phrase_aln_embeds.detach()) / len(obj_idx)
    ...
    def encode_image_phrase(self, image, phrase_embeds, phrase_masks):
        ...
        image_embeds = self.visual_encoder.forward_post(torch.cat([image_mid_embeds, phrase_embeds], dim=1), start_layer=6, mask=image_phrase_atts)
        image_feat = F.normalize(self.vision_proj(image_embeds[:, 0, :]), dim=-1)
        return image_feat
