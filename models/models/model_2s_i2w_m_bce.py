...
class ALBEF(nn.Module):
    ...
    def forward(self, image, text, alpha=0, idx=None, beta=1.):
        ...
        sim_i2w = torch.matmul(rep_text_embeds_m, image_embeds[:, 0, :].unsqueeze(-1)).squeeze(-1)  # N,L
        sim_i2w_m = torch.matmul(rep_text_embeds, image_embeds_m[:, 0, :].unsqueeze(-1)).squeeze(-1)
        
        # 将PAD的地方置为-10000
        loss_det = nn.BCEWithLogitsLoss(reduction='none')(sim_i2w, det_labels.float())
        loss_det = (loss_det * loss_mask).sum() / loss_mask.sum()
        loss_det_m = nn.BCEWithLogitsLoss(reduction='none')(sim_i2w_m, det_labels.float())
...
#         loss_i2w = self.weighted_contrastive_loss(sim_i2w/self.temp1, sim_i2w_targets, tau=8.0)
        return {'loss_ita': loss_ita, 'loss_i2w': loss_det}
