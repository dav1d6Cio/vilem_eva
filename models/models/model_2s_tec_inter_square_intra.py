...
class ALBEF(nn.Module):
    ...
    def forward(self, image, text, alpha=0, idx=None, beta=1.):
        ...
        # forward negative text
        text_neg_output = self.text_encoder.bert(text.rep_token_ids, attention_mask=text.attention_mask,
                                                 return_dict=True, mode='text')
        text_neg_feat = F.normalize(self.text_proj(text_neg_output.last_hidden_state[:, 0, :]), dim=-1)
        text_neg_feat_all = torch.stack([text_feat, text_neg_feat], dim=1)
        rep_word_feat = F.normalize(self.text_proj(text_neg_output.last_hidden_state[:, 1:, :]), dim=-1)  # N,L-1,C
        ...
        with torch.no_grad():
            ...
            # forward negative text
            text_neg_output_m = self.text_encoder_m.bert(text.rep_token_ids, attention_mask=text.attention_mask,
                                                         return_dict=True, mode='text')
            text_neg_feat_m = F.normalize(self.text_proj_m(text_neg_output_m.last_hidden_state[:, 0, :]), dim=-1)
            text_neg_feat_m_all = torch.stack([text_feat_m, text_neg_feat_m], dim=1)  # N,2,C
            rep_word_feat_m = F.normalize(self.text_proj_m(text_neg_output_m.last_hidden_state[:, 1:, :]), dim=-1)
        ...
        sim_neg = torch.matmul(image_feat.unsqueeze(1), text_neg_feat_m_all.transpose(1, 2)).squeeze(1)
        sim_neg_m = torch.matmul(image_feat_m.unsqueeze(1), text_neg_feat_all.transpose(1, 2)).squeeze(1)
        loss_neg = (sim_neg[:, 1:] - sim_neg[:, 0:1] + margin).clamp_(0.).sum(-1) #[non_zero_idx].mean()
        loss_neg = torch.pow(loss_neg, 2)[non_zero_idx].mean()
        loss_neg_m = (sim_neg_m[:, 1:] - sim_neg_m[:, 0:1] + margin).clamp_(0.).sum(-1)
        loss_neg_m = torch.pow(loss_neg_m, 2)[non_zero_idx].mean()
        loss_neg = (loss_neg + loss_neg_m) * beta
        ...
        # 必须要加clone，否则对sim_i2w_targets的操作会影响attention_mask
        sim_i2w_targets = text.attention_mask[:, 1:].clone()
        ...
        if 0 in sim_i2w_targets.sum(-1):
            ...
            sim_i2w_m = sim_i2w_m[non_zero_idx]
            sim_i2w_targets = sim_i2w_targets[non_zero_idx]
        loss_i2w = -torch.log((F.softmax(sim_i2w / self.temp1, dim=-1) * sim_i2w_targets).sum(1))
        if torch.any(torch.isnan(loss_i2w)):
            print(loss_i2w)
            print(text.input_ids[torch.isnan(loss_i2w)])
            print(text.rep_ids[torch.isnan(loss_i2w)])
            pdb.set_trace()
        loss_i2w = loss_i2w[~torch.isnan(loss_i2w)].mean()
        loss_i2w_m = -torch.log((F.softmax(sim_i2w_m / self.temp1, dim=-1) * sim_i2w_targets).sum(1))
        if torch.any(torch.isnan(loss_i2w_m)):
            print(loss_i2w_m)
            print(text.input_ids[torch.isnan(loss_i2w_m)])
            print(text.rep_ids[torch.isnan(loss_i2w_m)])
            pdb.set_trace()
        loss_i2w_m = loss_i2w_m[~torch.isnan(loss_i2w_m)].mean()
        ...
        if torch.isnan(loss_mlm):
            print('mlm loss is nan')
            pdb.set_trace()
        if torch.isnan(loss_det):
            print('det loss is nan')
            pdb.set_trace()

        return {'loss_ita': loss_ita, 'loss_tec': loss_mlm, 'loss_det': loss_det * self.lambda_ted, 'loss_inter': loss_neg, 'loss_intra': loss_i2w}
