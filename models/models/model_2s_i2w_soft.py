...
class ALBEF(nn.Module):
    ...
    def forward(self, image, text, alpha=0, idx=None, beta=1.):
        ...
        with torch.no_grad():
            ...
            sim_i2w_mm = torch.matmul(rep_word_feat_m, image_feat_m.unsqueeze(-1)).squeeze(-1)
            sim_i2w_mm = sim_i2w_mm + (1 - text.attention_mask[:, 1:]) * -1e9
            # 将sim_i2w_targets归一化
            sim_i2w_targets = text.attention_mask[:, 1:].clone()
            sim_i2w_targets[text.rep_ids[:, 1:]==1] = 0
            sim_i2w_targets = sim_i2w_targets / (sim_i2w_targets.sum(-1, keepdim=True) + 1e-9)
            # 计算i2w的soft target
            sim_i2w_targets = alpha * F.softmax(sim_i2w_mm/self.temp1, dim=1) + (1 - alpha) * sim_i2w_targets
        ...
        loss_i2w = -torch.sum(F.log_softmax(sim_i2w / self.temp1, dim=1) * sim_i2w_targets, dim=1).mean()
        loss_i2w_m = -torch.sum(F.log_softmax(sim_i2w_m / self.temp1, dim=1) * sim_i2w_targets, dim=1).mean()
        if loss_i2w > 100:
            pdb.set_trace()
